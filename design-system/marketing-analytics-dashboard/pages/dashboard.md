...
- Load Tremor charts lazily behind `Suspense`; reserve stable fallback space so analytics code does not delay the decision narrative or shift layout.
- On small screens, navigation becomes a keyboard-safe drawer: move focus inside on open, trap Tab, close on Escape, restore focus to the trigger, and hide the obscured main content from assistive technology.
